...
prompt_start = round(mean(correction.fitFLIM.peak - ...
                          2 * (correction.IRF.peak.PosInterp(:) - ...
                               correction.IRF.peak.risingHMinterp(:))));

% Find the last index of the prompt
prompt_end = ...
        round(mean(correction.fitFLIM.peak + ...
                   4 * round(correction.IRF.peak.fallingHMinterp(:) - ...
                             correction.IRF.peak.PosInterp(:))));

% Store the experimental prompt
correction.fitFLIM.expPrompt = ...
    double(correction.IRF.corrected(:, :, prompt_start : prompt_end));
...
time_bin = prompt_start : prompt_end;
time_bin = reshape(time_bin, 1, 1, numel(time_bin));
...
correction.fitFLIM.simPrompt = exGauss(time_bin, ...
...
correction.fitFLIM.avgExpPrompt = ...
    squeeze(sum(sum(correction.fitFLIM.avgExpPrompt, 1, 'omitnan'), ...
                2, 'omitnan'));
...
% Sum over the well fitted pixels
correction.fitFLIM.avgExpPromptFull = ...
    squeeze(sum(sum(correction.fitFLIM.avgExpPromptFull, 1, 'omitnan'), ...
                2, 'omitnan'));
...
if max(correction.fitFLIM.avgExpPromptFull) > intmax('uint16')
    correction.fitFLIM.avgExpPromptFull = ...
        correction.fitFLIM.avgExpPromptFull / ...
        max(correction.fitFLIM.avgExpPromptFull) * double(intmax('uint16'));
end
...
correction.fitFLIM.avgSimPrompt = exGauss(time_bin, ...
                                          h, ...
                                          correction.fitFLIM.mu, ...
                                          sigma, ...
                                          tau, ...
...
time_bin = 1 : size(correction.calibratedBins, 3);
% Simulate the average prompt
correction.fitFLIM.avgSimPromptFull = exGauss(time_bin, ...
                                              h, ...
                                              correction.fitFLIM.mu, ...
                                              sigma, ...
                                              tau, ...
